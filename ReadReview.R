...
for(i in 1:nrow(counts)){
  if(counts$Species[i]%in%species$Counts){
    counts$SPECIES[i]=as.character(species$AFS[which(counts$Species[i]==species$Counts)])
  } else {
    counts$SPECIES[i]=NA
  }
}
...
for(i in 1:nrow(weights)){
  if(weights$Species[i]%in%species$Counts){
    weights$SPECIES[i]=as.character(species$AFS[which(weights$Species[i]==species$Counts)])
  } else {
    weights$SPECIES[i]=NA
  }
}
...
for(i in 1:nrow(exacts)){
  if(exacts$Species[i]%in%species$Counts){
    exacts$SPECIES[i]=as.character(species$AFS[which(exacts$Species[i]==species$Counts)])
  } else {
    exacts$SPECIES[i]=NA
  }
}
