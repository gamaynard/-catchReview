  ...
  if(is.na(CodD$GEAR[i])==FALSE){
    if(CodD$GEAR[i]=="JIG"){
      CodD$POLRAT[i]=ifelse(
        (CodD$CD[i]+CodD$COD[i])==0,
        CodD$POLLOCK[i],
        CodD$POLLOCK[i]/(CodD$CD[i]+CodD$COD[i])
      )
    }
  }
}
...
Cod$DISCARDS=round(Cod$DISCARDS,0)
options(digits=0)
nr=c(
  "",
  round(colSums(Cod[,2:3]),0)
  )
Cod2=rbind(Cod,nr)
options(digits=0)
Cod2 %>% regulartable() %>% autofit() %>% set_formatter_type(Cod2,
fmt_double = "%.00f")
```